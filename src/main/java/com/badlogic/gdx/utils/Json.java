...
public class Json {
	...
	public void writeValue (Object value, Class knownType, Class elementType) {
		try {
			...
			if (value instanceof com.badlogic.gdx.utils.Queue) {
				if (knownType != null && actualType != knownType && actualType != com.badlogic.gdx.utils.Queue.class)
					throw new SerializationException("Serialization of a Queue other than the known type is not supported.\n"
						+ "Known type: " + knownType + "\nActual type: " + actualType);
				writeArrayStart();
				com.badlogic.gdx.utils.Queue queue = (com.badlogic.gdx.utils.Queue)value;
	...
	public <T> T readValue (Class<T> type, Class elementType, JsonValue jsonData) {
		...
		if (jsonData.isArray()) {
			...
			if (ClassReflection.isAssignableFrom(com.badlogic.gdx.utils.Queue.class, type)) {
				com.badlogic.gdx.utils.Queue result = type == com.badlogic.gdx.utils.Queue.class ? new com.badlogic.gdx.utils.Queue() : (Queue)newInstance(type);
				for (JsonValue child = jsonData.child; child != null; child = child.next)
